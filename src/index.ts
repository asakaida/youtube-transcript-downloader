...
async function handleListLanguages(videoId: string): Promise<void> {
	...
	for (const track of tracks) {
		// YouTube's track name often already contains "(auto-generated)"; only
		// append the marker when it is not already present, to avoid duplication.
		const alreadyLabeled = track.name.toLowerCase().includes("auto-generated");
		const autoLabel =
			track.isAutoGenerated && !alreadyLabeled ? " (auto-generated)" : "";
